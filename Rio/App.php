...
class App
{
    ...
    public function __construct(array $userSettings = array())
    {
        ...
        $this->container->singleton('environment', function ($c) {
            return \Rio\Environment::getInstance();
        });

        // Default request
        $this->container->singleton('request', function ($c) {
            return new \Rio\Http\Request($c['environment']);
        });

        // Default response
        $this->container->singleton('response', function ($c) {
            return new \Rio\Http\Response();
        });

        // Default router
        $this->container->singleton('router', function ($c) {
            return new \Rio\Router();
        });
        ...
        $this->container->singleton('view', function ($c) {
            ...
            $view = ($viewClass instanceOf \Rio\View) ? $viewClass : new $viewClass;
        ...
        $this->container->singleton('logWriter', function ($c) {
            ...
            return is_object($logWriter) ? $logWriter : new \Rio\LogWriter($c['environment']['slim.errors']);
        });

        // Default log
//        $this->container->singleton('log', function ($c) {
//            $log = new \Rio\Log($c['logWriter']);
...
//        });
        ...
        $this->add(new \Rio\Middleware\Flash());
        $this->add(new \Rio\Middleware\MethodOverride());
    ...
    protected function mapRoute($args)
    {
        ...
        $route = new \Rio\Route($pattern, $callable, $this->settings['routes.case_sensitive']);
    ...
    public function get()
    {
        ...
        return $this->mapRoute($args)->via(\Rio\Http\Request::METHOD_GET, \Rio\Http\Request::METHOD_HEAD);
    }
    ...
    public function post()
    {
        ...
        return $this->mapRoute($args)->via(\Rio\Http\Request::METHOD_POST);
    }
    ...
    public function put()
    {
        ...
        return $this->mapRoute($args)->via(\Rio\Http\Request::METHOD_PUT);
    }
    ...
    public function patch()
    {
        ...
        return $this->mapRoute($args)->via(\Rio\Http\Request::METHOD_PATCH);
    }
    ...
    public function delete()
    {
        ...
        return $this->mapRoute($args)->via(\Rio\Http\Request::METHOD_DELETE);
    }
    ...
    public function options()
    {
        ...
        return $this->mapRoute($args)->via(\Rio\Http\Request::METHOD_OPTIONS);
    }
     ...
     *
     * @param  string|\Rio\View $viewClass The name or instance of a \Rio\View subclass
     * @return \Rio\View
     */
    public function view($viewClass = null)
    {
        if (!is_null($viewClass)) {
            ...
            if ($viewClass instanceOf \Rio\View) {
                $this->view = $viewClass;
            } else {
    ...
    public function getCookie($name, $deleteIfInvalid = true)
    {
        ...
        if ($this->config('cookies.encrypt')) {
            $value = \Rio\Http\Util::decodeSecureCookie(
                $value,
         ...
         * \Rio\Http\Util::decodeSecureCookie -  is able
     ...
     *
     * @throws \Rio\Exception\Stop
     */
    public function stop()
    {
        throw new \Rio\Exception\Stop();
    }
     ...
     *
     * @throws \Rio\Exception\Pass
     */
    public function pass()
    {
        $this->cleanBuffer();
        throw new \Rio\Exception\Pass();
    }
     ...
     *
     * @param \Rio\Middleware
     */
    public function add(\Rio\Middleware $newMiddleware)
    {
    ...
    public function run()
    {
        set_error_handler(array('\Rio\Rio', 'handleErrors'));

        //Apply final outer middleware layers
        if ($this->config('debug')) {
            //Apply pretty exceptions only in debug to avoid accidental information leakage in production
            $this->add(new \Rio\Middleware\PrettyExceptions());
        }
        ...
        \Rio\Http\Util::serializeCookies($headers, $this->response->cookies, $this->settings);

        //Send headers
        if (headers_sent() === false) {
            //Send status
            if (strpos(PHP_SAPI, 'cgi') === 0) {
                header(sprintf('Status: %s', \Rio\Http\Response::getMessageForCode($status)));
            } else {
                header(sprintf('HTTP/%s %s', $this->config('http.version'), \Rio\Http\Response::getMessageForCode($status)));
            }
